...
FragTrap::~FragTrap () { print ("FragTrap destructor called"); }

void
FragTrap::highFivesGuys ()
{
  if (getHitPoints () <= 0)
    print ("FragTrap " << getName ()
                       << " is dead and can't request a high five!");
  else if (getEnergyPoints () <= 0)
    print ("FragTrap " << getName ()
                       << " is out of energy and can't request a high five!");
  else
